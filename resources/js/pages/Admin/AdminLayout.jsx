...
export default function AdminLayout({ children }) {
    ...
    const SideLink = ({ to, icon, label, badge: badgeCount }) => (
        <NavLink to={to} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition ${isActive(to) ? 'bg-white/10 text-white' : 'hover:bg-white/10 text-white/80 hover:text-white'}`}>
            <i className={`${icon} w-5`}></i>
            <span className="flex-1">{label}</span>
            {badgeCount > 0 && <span className="inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold bg-yellow-400 text-yellow-900 rounded-full">{badgeCount}</span>}
        </NavLink>
    );
        ...
        <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Inter', sans-serif" }}>
            ...
            <aside className="hidden md:flex fixed inset-y-0 left-0 z-50 w-64 bg-[#E67E22] text-white flex-col h-screen">
                ...
                <nav className="p-4 space-y-1 flex-1 overflow-y-auto">
                    <SideLink to="/admin/dashboard" icon="fas fa-tachometer-alt" label="Dashboard" />
                    <SideLink to="/admin/pendaftaran" icon="fas fa-users" label="Data Pendaftar" />
                    ...
                    {isAdminRole && (
                        <>
                            ...
                            <SideLink to="/admin/biaya" icon="fas fa-money-bill" label="Biaya" />
                            <SideLink to="/admin/perlengkapan" icon="fas fa-box" label="Perlengkapan" />
                            <SideLink to="/admin/transaksi" icon="fas fa-money-bill-wave" label="Transaksi" />
                            <SideLink to="/admin/pos-keuangan" icon="fas fa-chart-pie" label="Pos Keuangan" />
                        </>
                    )}
                    ...
                    <SideLink to="/admin/beasiswa" icon="fas fa-graduation-cap" label="Beasiswa" />
                    <SideLink to="/admin/kontak" icon="fas fa-phone-alt" label="Kontak" />
                    <SideLink to="/admin/pengaturan" icon="fas fa-cog" label="Pengaturan" />
                    {isAdminRole && <SideLink to="/admin/aktivitas" icon="fas fa-history" label="Log Aktivitas" />}
                    ...
                    {isSuperAdmin && (
                        <>
                            ...
                            <SideLink to="/admin/kelola-user" icon="fas fa-user-cog" label="Kelola User" />
                        </>
                    )}
                </nav>
