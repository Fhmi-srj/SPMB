...
export default function Profil() {
    ...
    const roleBadge = () => {
        ...
        return <span className={`inline-block px-2.5 py-0.5 text-xs sm:text-sm font-semibold rounded-full ${color}`}>{label}</span>;
    };
    ...
    return (
        <div>
            <div className="mb-4 sm:mb-6">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Profil Admin</h2>
                <p className="text-gray-500 text-xs sm:text-sm">Kelola informasi akun Anda</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                {/* Profile Card */}
                <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                    <div className="bg-[#E67E22]/5 px-3 py-2 sm:px-4 sm:py-3 border-b border-gray-100">
                        <h3 className="font-semibold text-sm sm:text-base text-[#E67E22]"><i className="fas fa-user mr-2"></i>Informasi Profil</h3>
                    </div>
                    <form onSubmit={handleUpdateProfile} className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                        <div>
                            <label className="block text-[11px] sm:text-sm font-medium text-gray-600 mb-0.5">Role</label>
                            {roleBadge()}
                        </div>
                        <div>
                            <label className="block text-[11px] sm:text-sm font-medium text-gray-600 mb-0.5">Nama Lengkap</label>
                        ...
                        <div>
                            <label className="block text-[11px] sm:text-sm font-medium text-gray-600 mb-0.5">Username</label>
                ...
                {/* Password Card */}
                <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                    <div className="bg-yellow-50 px-3 py-2 sm:px-4 sm:py-3 border-b border-gray-100">
                        <h3 className="font-semibold text-sm sm:text-base text-yellow-700"><i className="fas fa-key mr-2"></i>Ubah Password</h3>
                    </div>
                    <form onSubmit={handleChangePassword} className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                        <div>
                            <label className="block text-[11px] sm:text-sm font-medium text-gray-600 mb-0.5">Password Saat Ini</label>
                            <div className="relative">
                                <input type={showPw.current ? 'text' : 'password'} value={pwForm.current_password} onChange={e => setPwForm({ ...pwForm, current_password: e.target.value })} required
                                    className="w-full px-2.5 py-1.5 sm:px-4 sm:py-2 pr-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E67E22] focus:border-transparent outline-none text-xs sm:text-sm" />
                                <button type="button" onClick={() => togglePw('current')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                    <i className={`fas ${showPw.current ? 'fa-eye-slash' : 'fa-eye'} text-xs sm:text-sm`}></i>
                                </button>
                            </div>
                        </div>
                        <div>
                            <label className="block text-[11px] sm:text-sm font-medium text-gray-600 mb-0.5">Password Baru</label>
                            <div className="relative">
                                ...
                                <button type="button" onClick={() => togglePw('new')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                    <i className={`fas ${showPw.new ? 'fa-eye-slash' : 'fa-eye'} text-xs sm:text-sm`}></i>
                                </button>
                            </div>
                            <p className="text-[10px] sm:text-xs text-gray-500 mt-0.5">Minimal 6 karakter</p>
                        </div>
                        <div>
                            <label className="block text-[11px] sm:text-sm font-medium text-gray-600 mb-0.5">Konfirmasi Password Baru</label>
                            <div className="relative">
                                ...
                                <button type="button" onClick={() => togglePw('confirm')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                    <i className={`fas ${showPw.confirm ? 'fa-eye-slash' : 'fa-eye'} text-xs sm:text-sm`}></i>
                                </button>
                            </div>
                        </div>
