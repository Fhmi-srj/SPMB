...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_perlengkapan') {
        ...
        $stmt = $conn->prepare("INSERT INTO perlengkapan_items (nama_item, nominal, urutan) VALUES (?, ?, ?)");
        $stmt->bind_param("sii", $nama_item, $nominal, $urutan);

        if ($stmt->execute()) {
            $message = 'Perlengkapan berhasil ditambahkan!';
        } else {
            $error = 'Gagal menambahkan perlengkapan!';
        }
    }

    if ($action === 'edit_perlengkapan') {
        $id = intval($_POST['id']);
        $nama_item = sanitize($conn, $_POST['nama_item']);
        $nominal = intval($_POST['nominal'] ?? 0);
        $urutan = intval($_POST['urutan'] ?? 0);

        $stmt = $conn->prepare("UPDATE perlengkapan_items SET nama_item=?, nominal=?, urutan=? WHERE id=?");
        $stmt->bind_param("siii", $nama_item, $nominal, $urutan, $id);

        if ($stmt->execute()) {
            $message = 'Perlengkapan berhasil diupdate!';
        } else {
            $error = 'Gagal mengupdate perlengkapan!';
        }
    }

    if ($action === 'delete_perlengkapan') {
        $id = intval($_POST['id']);
        $stmt = $conn->prepare("DELETE FROM perlengkapan_items WHERE id = ?");
        $stmt->bind_param("i", $id);

        if ($stmt->execute()) {
            $message = 'Perlengkapan berhasil dihapus!';
        } else {
            $error = 'Gagal menghapus perlengkapan!';
        }
    }
...
$perlengkapanResult = $conn->query("SELECT * FROM perlengkapan_items WHERE aktif = 1 ORDER BY urutan ASC");
$perlengkapanList = [];
while ($row = $perlengkapanResult->fetch_assoc()) {
    $perlengkapanList[] = $row;
}
    ...
    <main class="main-content p-4 md:p-6">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        foreach ($biayaList as $row):
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600 text-right">
                                    <?= $row['biaya_pondok'] > 0 ? 'Rp' . number_format($row['biaya_pondok'], 0, ',', '.') : '-' ?>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600 text-right">
                                    <?= $row['biaya_smp'] > 0 ? 'Rp' . number_format($row['biaya_smp'], 0, ',', '.') : '-' ?>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600 text-right">
                                    <?= $row['biaya_ma'] > 0 ? 'Rp' . number_format($row['biaya_ma'], 0, ',', '.') : '-' ?>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    ...
                                    <button
                                        onclick="deleteItem(<?= $row['id'] ?>, '<?= htmlspecialchars($row['nama_item']) ?>')"
                        ...
                        <tr class="bg-primary text-white font-bold">
                            ...
                            <td class="px-4 py-3 text-sm text-right">
                                Rp<?= number_format($totals['pondok'], 0, ',', '.') ?></td>
                            <td class="px-4 py-3 text-sm text-right">Rp<?= number_format($totals['smp'], 0, ',', '.') ?>
                            </td>
                            <td class="px-4 py-3 text-sm text-right">Rp<?= number_format($totals['ma'], 0, ',', '.') ?>
                            </td>
                            <td></td>
                        </tr>
                    </tbody>
        ...
        <div class="mt-8">
            <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="text-xl font-bold text-gray-800">Perlengkapan Tambahan</h3>
                    <p class="text-gray-500 text-sm">Kelola perlengkapan tambahan (kasur, almari, bantal, dll)</p>
                </div>
                <button onclick="openModal('addPerlengkapanModal')"
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Urutan
                                </th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            <?php
                            $no = 1;
                            foreach ($perlengkapanList as $item):
                                ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 text-sm text-gray-500"><?= $no++ ?></td>
                                    <td class="px-4 py-3 text-sm font-medium text-gray-800">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <button onclick="editPerlengkapan(<?= htmlspecialchars(json_encode($item)) ?>)"
                                            class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button
                                            onclick="deletePerlengkapan(<?= $item['id'] ?>, '<?= htmlspecialchars($item['nama_item']) ?>')"
                                            class="p-2 text-red-600 hover:bg-red-100 rounded-lg transition">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </td>
    ...
    <div class="modal-container bg-white rounded-xl max-w-md w-full">
        ...
        <form method="POST">
            <div class="modal-body space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Item</label>
                    <input type="text" name="nama_item" id="editNamaItem" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                </div>
                <div class="grid grid-cols-3 gap-3">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Pondok</label>
                        <input type="number" name="biaya_pondok" id="editBiayaPondok"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">SMP</label>
                        <input type="number" name="biaya_smp" id="editBiayaSmp"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">MA</label>
                        <input type="number" name="biaya_ma" id="editBiayaMa"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                    </div>
                </div>
                <div class="form-group">
...
<!-- Add Perlengkapan Modal -->
<div id="addPerlengkapanModal"
    class="modal-overlay fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4">
    <div class="modal-container bg-white rounded-xl max-w-md w-full">
        <div class="modal-header flex items-center justify-between">
            <h3>Tambah Perlengkapan</h3>
            <button onclick="closeModal('addPerlengkapanModal')" class="close-btn"><i class="fas fa-times"></i></button>
        </div>
        <form method="POST">
            <div class="modal-body space-y-4">
                <input type="hidden" name="action" value="add_perlengkapan">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Perlengkapan</label>
                    <input type="text" name="nama_item" required placeholder="Contoh: Kasur, Almari, Bantal"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                </div>
                <div>
            ...
            <div class="modal-footer">
                <button type="button" onclick="closeModal('addPerlengkapanModal')" class="btn btn-cancel">Batal</button>
                <button type="submit" class="btn btn-primary">Simpan</button>
            </div>
        </form>
    </div>
...
<!-- Edit Perlengkapan Modal -->
<div id="editPerlengkapanModal"
    class="modal-overlay fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4">
    <div class="modal-container bg-white rounded-xl max-w-md w-full">
        <div class="modal-header flex items-center justify-between">
            <h3>Edit Perlengkapan</h3>
            <button onclick="closeModal('editPerlengkapanModal')" class="close-btn"><i
                    class="fas fa-times"></i></button>
        </div>
        <form method="POST">
            <div class="modal-body space-y-4">
                <input type="hidden" name="action" value="edit_perlengkapan">
                <input type="hidden" name="id" id="editPerlengkapanId">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Perlengkapan</label>
                    <input type="text" name="nama_item" id="editPerlengkapanNama" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nominal (Rp)</label>
                    <input type="number" name="nominal" id="editPerlengkapanNominal" min="0"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Urutan</label>
                    <input type="number" name="urutan" id="editPerlengkapanUrutan" min="0"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" onclick="closeModal('editPerlengkapanModal')"
...
<!-- Delete Perlengkapan Modal -->
<div id="deletePerlengkapanModal"
    class="modal-overlay fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4">
    <div class="modal-container bg-white rounded-xl max-w-sm w-full p-6 text-center">
        <div class="delete-modal-icon">
            <i class="fas fa-trash-alt"></i>
        </div>
        <h3 class="font-bold text-lg text-gray-800 mb-2">Hapus Perlengkapan?</h3>
        <p class="text-gray-500 text-sm mb-6">Yakin ingin menghapus <strong id="deletePerlengkapanName"></strong>?</p>
        <form method="POST" class="flex gap-3">
            <input type="hidden" name="action" value="delete_perlengkapan">
            <input type="hidden" name="id" id="deletePerlengkapanId">
            <button type="button" onclick="closeModal('deletePerlengkapanModal')" class="btn btn-cancel">Batal</button>
            <button type="submit" class="btn btn-danger">Hapus</button>
        </form>
    </div>
</div>
...
<script>
    ...
    function editPerlengkapan(data) {
        document.getElementById('editPerlengkapanId').value = data.id;
        document.getElementById('editPerlengkapanNama').value = data.nama_item;
        document.getElementById('editPerlengkapanNominal').value = data.nominal;
        document.getElementById('editPerlengkapanUrutan').value = data.urutan;
        openModal('editPerlengkapanModal');
    }

    function deletePerlengkapan(id, name) {
        document.getElementById('deletePerlengkapanId').value = id;
        document.getElementById('deletePerlengkapanName').textContent = name;
        openModal('deletePerlengkapanModal');
    }
